		...
		try {
			if (retrievedProduct.next()) {
				if(!unit.equals(retrievedProduct.getString(3))){
					return false; //sl�nga error ist�llet?
				}
			}
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			...
			try {
				...
				while (temp.next()) {
					try{
						...
						if(uncertain){
							//os�kerhet om hur mycket av denna product vi har.
							Product tempProd = new Product(temp.getString(1), temp.getInt(3), temp.getString(5), uncertain);
							tempList.add(tempProd);
						} else {
							canMakeRecipe = false;
							break; //ingen os�kerhet och vi kan inte g�ra denna produkt.
						}
					} catch (Exception e){
						//null v�rde i databasen dvs saknade denna ingrediens helt och kan inte g�ra detta recept alls.
						canMakeRecipe = false;
						break;
					}
